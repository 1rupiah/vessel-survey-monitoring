...
export default function App() {
  ...
  function exportCSV() {
    ...
    track.forEach((p) => {
      csv += `${p.ts},${p.lat.toFixed(6)},${p.lon.toFixed(6)},${p.sog.toFixed(2)},${p.depth.toFixed(1)}\n`;
    });
  ...
  const btnStyle = (type, active = true) => {
    if (type === "follow") {
      return {
        background: active ? "#00C853" : "#F44336",
        border: `1px solid ${active ? "#00C853" : "#F44336"}`,
        color: "#FFFFFF",
    ...
    if (type === "record") {
      return {
        background: active ? "#FF1744" : "#616161",
        border: `1px solid ${active ? "#FF1744" : "#616161"}`,
        color: "#FFFFFF",
        fontFamily: "var(--font-ui)",
        fontSize: 12,
        fontWeight: 700,
        ...
        textTransform: "uppercase",
        borderRadius: 0,
        boxShadow: active ? "0 0 12px rgba(255,23,68,0.5)" : "none",
        transition: "all 0.15s ease",
      };
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", gap: 24, alignItems: "center" }}>
          {[
            ...
            { label: "SONAR", ok: !!data },
            {
              label: source === "ws" ? "WS CONNECTED" : "SIM MODE",
              ok: connected,
              blink: source === "ws",
            },
            {
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, position: "relative", overflow: "hidden" }}>
          ...
          >
            ...
            <button
              style={btnStyle("record", recording)}
              onClick={() => setRecording((r) => !r)}
            >
              {recording ? "🔴 REC ON" : "⚫ REC OFF"}
            </button>

            <button style={btnStyle("reset")} onClick={() => setFollow(true)}>
              ↺ RESET
            </button>
          </div>
          ...
          >
            TRACK PTS:{" "}
            <span style={{ color: "var(--accent)" }}>{track.length}</span>
          </div>
        </div>
      ...
      >
        <span>
          VESSEL: <span style={{ color: "var(--accent)" }}>MV SURVEY-01</span>
        </span>
        <span style={{ color: "var(--dim)" }}>|</span>
        <span>
          PROJ: <span style={{ color: "var(--accent)" }}>BATHY-2025-IDN</span>
        </span>
        <span style={{ color: "var(--dim)" }}>|</span>
        <span>
          ACTIVE LINE:{" "}
